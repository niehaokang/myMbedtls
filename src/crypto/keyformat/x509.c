...
static const struct {
	...
	{ { 0xfb, 0x08, 0x40, 0x61, 0x82, 0xd2, 0x39, 0x44, 0x1f,
		0xbb, 0x69, 0x91, 0xfc, 0x5c, 0xca, 0xce }, EXT_TVM_NAME },
	{ { 0x1d, 0x21, 0x59, 0xe9, 0x51, 0xe5, 0xf4, 0x23, 0xc8,
		0x0d, 0x80, 0x09, 0x30, 0xfe, 0x33, 0x08 }, EXT_COUNTER },
	{ { 0x6d, 0x29, 0x48, 0x15, 0xf2, 0x10, 0x36, 0x1c, 0xe2,
        0x48, 0xbb, 0xdb, 0xac, 0xf7, 0x9e, 0xfc }, EXT_UUID },
...
*/
				...
				break;
			case EXT_TVM_NAME:
				extensions->tvmNameLen = fullExtLen - (p - extStart)+1;
				memcpy(extensions->tvmName,p,extensions->tvmNameLen);
				extensions->tvmName[extensions->tvmNameLen-1] = '\0';
				p = p + (fullExtLen - (p - extStart));
				break;
			case EXT_COUNTER:
				*(char*)(&extensions->counter) = *(p+3);
				*((char*)(&extensions->counter)+1) = *(p+2);
				*((char*)(&extensions->counter)+2) = *(p+1);
				*((char*)(&extensions->counter)+3) = *p;
				//memcpy(&extensions->counter,p,4);
				p = p + (fullExtLen - (p - extStart));
				break;
			case EXT_UUID:
				extensions->uuidLen = fullExtLen - (p - extStart)/* + 1*/;
				memcpy(extensions->uuid, p, extensions->uuidLen);
				// extensions->uuid[extensions->uuidLen - 1] = '\0';
				p = p + (fullExtLen - (p - extStart));
				break;
			default:
...
static int32 validateDateRange(psX509Cert_t *cert)
{
#if (defined POSIX) || (defined SOTER)
	struct tm		t;
	time_t			rawtime;
	unsigned char	*c;
	unsigned int	y;
...
#ifdef SOTER
	psTime_t 		pst;
	if (psGetTime(&pst, NULL) < 0 ) {
		printf("psGetTime: %d:%d\n", pst.tv_sec, pst.tv_usec);
		return PS_FAILURE;
	}

	if (gmtime_r(&pst.tv_sec, &t) == NULL) {
		psTraceCrypto("call gmtime_r failed !!\n");
		return PS_FAILURE;
	}
	...
	//		t.tm_year, t.tm_mon, t.tm_yday, t.tm_hour, t.tm_min, t.tm_sec);
#else
	time(&rawtime);
...
*/
		if (ic == sc) {
			*foundIssuer = ic;
			...
			psTraceCrypto("Single self-signed test completed\n");
		} else if (ic == issuerCert) {
			*foundIssuer = ic;
			ic = NULL; /* If issuerCert was used, that is always final test */
			psTraceCrypto("If issuerCert was used, that is always final test\n");
		} else {
			sc = ic;
			ic = sc->next;
			if (ic == NULL) { /* Reached end of chain */
				*foundIssuer = ic;
				...
				psTraceCrypto("Self-signed test on final subectCert chain\n");
			}
		}
...
static int x509_write_version( unsigned char **p, unsigned char *start,
		int32 version ) {
	...
	/*
		Version  ::=  INTEGER  {  v1(0), v2(1), v3(2)  }
	*/
	__ASN_CHECK__( len, setAsnInteger( p, start, version ) );
	__ASN_CHECK__( len, setAsnLength( p, start, len ) );
	__ASN_CHECK__( len, setAsnTag( p, start,  (ASN_CONTEXT_SPECIFIC | ASN_CONSTRUCTED | 0) ) );

	return( (int) len );
}

static int x509_write_serial_number( unsigned char **p, unsigned char *start,
		unsigned char *serialNumber, uint32 serialNumberLen ) {
	...
	/*
		...
		will get written to the parse warning flag
	*/
	if ( serialNumber != NULL && serialNumberLen > 0 )
	{
		__ASN_CHECK__( len, setAsnRawData( p, start, serialNumber, serialNumberLen ) );
	}

	__ASN_CHECK__( len, setAsnLength( p, start, serialNumberLen ) );
	__ASN_CHECK__( len, setAsnTag( p, start, ASN_INTEGER ) );

	return( (int) len );
}

static int x509_write_alorithm_identifier( unsigned char **p, unsigned char *start,
		int32 algorithm ) {

	int ret;
	uint32 len = 0, oid_len = 0;
	unsigned char *oid;
	...
	*/
	...
	case OID_RSA_KEY_ALG:
		oid = "\x2A\x86\x48\x86\xF7\x0D\x01\x01\x01";//1.2.840.113549.1.1.1
		oid_len = 0x09;
		break;
	/////////////////////////////////////////////////
	// Signature...
	case OID_SHA1_RSA_SIG:
		oid = "\x2A\x86\x48\x86\xF7\x0D\x01\x01\x05";
		oid_len = 0x09;
		break;
	case OID_SHA256_RSA_SIG:
		oid = "\x2A\x86\x48\x86\xF7\x0D\x01\x01\x0B";
		oid_len = 0x09;
		break;
	case OID_SHA384_RSA_SIG:
		oid = "\x2A\x86\x48\x86\xF7\x0D\x01\x01\x0C";
		oid_len = 0x09;
		break;
	case OID_SHA512_RSA_SIG:
		oid = "\x2A\x86\x48\x86\xF7\x0D\x01\x01\x0D";/* 42.134.72.134.247.13.1.1.13 */
		oid_len = 0x09;
	case OID_ECDSA_KEY_ALG:                        
		oid = "\x2B\x81\x04\x00\x21";//43.129.4.0.33//"\x2A\x86\x48\xCE\x3D\x02\x01";/* 1.2.840.10045.2.1 */ //42.134.72.206.3d.2.1
		oid_len = 0x5;
	break;
	/////////////////////////////////////////////////
	default:
	...
	__ASN_CHECK__( len, setAsnSequence( p, start, len ) );

	return( (int) len );
}

static int x509_write_attributes( unsigned char **p, unsigned char *start,
		x509DNattributes_t *attrib ) {
	 ...
	 */
	struct __attribs__ {
		unsigned char *oid; int type; char *s;
	} attribs[] = {
		{ (unsigned char *)"\x55\x04\x03", attrib->commonNameType, attrib->commonName },
		{ (unsigned char *)"\x55\x04\x0B", attrib->orgUnitType, attrib->orgUnit },
		{ (unsigned char *)"\x55\x04\x0A", attrib->organizationType, attrib->organization },
		{ (unsigned char *)"\x55\x04\x07", attrib->localityType, attrib->locality },
		{ (unsigned char *)"\x55\x04\x08", attrib->stateType, attrib->state },
		{ (unsigned char *)"\x55\x04\x06", attrib->countryType, attrib->country }
	};

	for ( i = 0; i < sizeof(attribs)/sizeof(attribs[0]); i++ )
	{
		uint32 sub_len = 0;

		if ( attribs[i].s != NULL )
		{
			__ASN_CHECK__( sub_len, setAsnRawData( p, start, (unsigned char *)attribs[i].s, (uint32)strlen(attribs[i].s) ) );
			__ASN_CHECK__( sub_len, setAsnLength( p, start, sub_len ) );
			__ASN_CHECK__( sub_len, setAsnTag( p, start, attribs[i].type ) );

			__ASN_CHECK__( sub_len, setAsnOID( p, start, attribs[i].oid, 0x03 ) );
			__ASN_CHECK__( sub_len, setAsnSequence( p, start, sub_len ) );

			__ASN_CHECK__( sub_len, setAsnLength( p, start, sub_len ) );
			__ASN_CHECK__( sub_len, setAsnTag( p, start, (ASN_SET | ASN_CONSTRUCTED) ) );
		}
...
static int x509_write_time( unsigned char **p, unsigned char *start,
                            const char *time_s, int32  time_type_s,
							const char *time_e, int32  time_type_e ) {
    ...
    */
    if ( time_s != NULL && time_e != NULL )
    {
    	...
    	__ASN_CHECK__( sub_len, setAsnRawData( p, start,(unsigned char *)&time_e[0], strlen(time_e) ) );
		__ASN_CHECK__( sub_len, setAsnLength( p, start, sub_len ) );
		__ASN_CHECK__( sub_len, setAsnTag( p, start, time_type_e ) );
		
		len = sub_len + len; 
		sub_len = 0;
		__ASN_CHECK__( sub_len, setAsnRawData( p, start,(unsigned char *)&time_s[0], strlen(time_s) ) );
		__ASN_CHECK__( sub_len, setAsnLength( p, start, sub_len ) );
		__ASN_CHECK__( sub_len, setAsnTag( p, start, time_type_s ) );
...
int x509_write_publickey( psPool_t *pool, unsigned char **p, unsigned char *start,
	...
	*/
	...
	else if ( pubKeyAlgorithm == OID_RSA_KEY_ALG )
	{
		__ASN_CHECK__( len, setAsnBig( pool, p, start, &publicKey->key->rsa.e) );
		__ASN_CHECK__( len, setAsnBig( pool, p, start, &publicKey->key->rsa.N) );
		__ASN_CHECK__( len, setAsnSequence( p, start, len ) );
		__ASN_CHECK__( len, setAsnTag( p, start, 0x00 ) );
		__ASN_CHECK__( len, setAsnLength( p, start, len ) );
		__ASN_CHECK__( len, setAsnTag( p, start, ASN_BIT_STRING ) );
	}

	__ASN_CHECK__( len, x509_write_alorithm_identifier( p, start,  pubKeyAlgorithm ) );
	__ASN_CHECK__( len, setAsnSequence( p, start, len ) );
	
	return( (int) len );
}

static int x509_write_basic_constraints( unsigned char **p, unsigned char *start,
		x509extBasicConstraints_t *bc, unsigned char *oid, uint32 oid_len, int critical ) {

	int ret;
	uint32 len = 0;

	if( bc->cA && bc->pathLenConstraint > 127 )
		return( -1 );
	...
	if( bc->cA )
	{
		...
		{
			__ASN_CHECK__( len, setAsnLength( p, start, bc->pathLenConstraint ) );
		}

		__ASN_CHECK__( len, setAsnBool( p, start, 1 ) );
	}
	...
	if (critical != 0)
	{
		__ASN_CHECK__( len, setAsnBool( p, start, 0x01 ) );
	}
...
static int x509_write_subject_name( unsigned char **p, unsigned char *start,
		x509GeneralName_t *san, unsigned char *oid, uint32 oid_len, int critical ) {
	...
	while  ( san != NULL )
	{
		if ( san->data != NULL && san->dataLen > 0 ) {

			uint32 sub_len = 0;

			/* This guarantees data is null terminated, even for non IA5Strings */
			__ASN_CHECK__( sub_len, setAsnRawData( p, start, san->data, san->dataLen ) );
			__ASN_CHECK__( sub_len, setAsnLength( p, start, sub_len ) );

			if (san->id == GN_OTHER)
			{
				__ASN_CHECK__( sub_len, setAsnTag( p, start, ASN_NULL ) );
				__ASN_CHECK__( sub_len, setAsnTag( p, start, 0xA0 ) );
				__ASN_CHECK__( sub_len, setAsnOID( p, start, san->oid, san->oidLen ) );
			}

			__ASN_CHECK__( sub_len, setAsnTag( p, start, san->id ) );
			len = sub_len + len;
		}
...
static int x509_write_subject_key_id( unsigned char **p, unsigned char *start,
		x509extSubjectKeyId_t *sk, unsigned char *oid, uint32 oid_len, int critical ) {

	int ret;
	uint32 len = 0;

	if ( sk->id != NULL && sk->len > 0 )
	{
		__ASN_CHECK__( len, setAsnRawData( p, start, sk->id, sk->len ) );
		__ASN_CHECK__( len, setAsnLength( p, start, len ) );
		__ASN_CHECK__( len, setAsnTag( p, start, ASN_OCTET_STRING ) );
	}
...
static int x509_write_auth_key_id( unsigned char **p, unsigned char *start,
		x509extAuthKeyId_t *ak, unsigned char *oid, uint32 oid_len, int critical ) {

	int ret;
	uint32 len = 0;
	uint32 sub_len = 0;

	if (ak->serialNum != NULL && ak->serialNumLen > 0)
	{
		sub_len = 0;
		__ASN_CHECK__( sub_len, setAsnRawData( p, start, ak->serialNum, ak->serialNumLen ) );
		__ASN_CHECK__( sub_len, setAsnLength( p, start, sub_len ) );
		__ASN_CHECK__( sub_len, setAsnTag( p, start, ASN_INTEGER ) );
	}
	len = sub_len + len;

	if ( ak->attribs.country != NULL ||
		 ak->attribs.state != NULL ||
		 ak->attribs.locality != NULL ||
		 ak->attribs.organization != NULL ||
		 ak->attribs.orgUnit != NULL ||
		 ak->attribs.commonName )
	{
		sub_len = 0;

		__ASN_CHECK__( sub_len, x509_write_attributes( p, start, &ak->attribs ) );
		__ASN_CHECK__( sub_len, setAsnLength( p, start, sub_len ) );
		__ASN_CHECK__( sub_len, setAsnTag( p, start, (ASN_CONTEXT_SPECIFIC | ASN_PRIMITIVE | 0) ) );
		__ASN_CHECK__( sub_len, setAsnLength( p, start, sub_len ) );
		__ASN_CHECK__( sub_len, setAsnTag( p, start, (ASN_CONTEXT_SPECIFIC | ASN_CONSTRUCTED | 1) ) );

	}
	len = sub_len + len;

	if (ak->keyId != NULL && ak->keyLen > 0)
	{
		sub_len = 0;
		__ASN_CHECK__( sub_len, setAsnRawData( p, start, ak->keyId, ak->keyLen ) );
		__ASN_CHECK__( sub_len, setAsnLength( p, start, sub_len ) );
		__ASN_CHECK__( sub_len, setAsnTag( p, start, (ASN_CONTEXT_SPECIFIC | ASN_PRIMITIVE | 0) ) );
	}
	len = sub_len + len;
...
static int x509_write_key_usage( unsigned char **p, unsigned char *start,
		uint32 keyUsageFlags, unsigned char *oid, uint32 oid_len, int critical ) {

	int ret;
	uint32 len = 0;
	unsigned char __data[3] = {0};

	if ( !(keyUsageFlags & KEY_USAGE_DECIPHER_ONLY) )
	{
		/* then there are one bytes of flags */
		__data[1] = (keyUsageFlags & 0xFF);

		__ASN_CHECK__( len, setAsnRawData( p, start, __data, 2 ) );
		__ASN_CHECK__( len, setAsnLength( p, start, len ) );
		__ASN_CHECK__( len, setAsnTag( p, start, ASN_BIT_STRING ) );
	}
	else
	{
		/* then there are two bytes of flags */
		__data[0] = 0x07;
		__data[2] = (KEY_USAGE_DECIPHER_ONLY >> 8);

		__ASN_CHECK__( len, setAsnRawData( p, start, __data, 3 ) );
		__ASN_CHECK__( len, setAsnLength( p, start, len ) );
		__ASN_CHECK__( len, setAsnTag( p, start, ASN_BIT_STRING ) );
	}
...
static int x509_write_ecc_oid(unsigned char **p, unsigned char *start,unsigned char *oid,uint32 oidlen,unsigned char *curve_oid,uint32 curve_oidlen)
{
	int ret;
	int len;
	__ASN_CHECK__( len, setAsnOID( p, start, curve_oid, curve_oidlen ) );

	__ASN_CHECK__( len, setAsnOID( p, start, oid, oidlen ) );
...
int x509_export_ecc_key( psPool_t *pool, unsigned char **p, unsigned char *start,
	psPubKey_t *publicKey, int32 pubKeyAlgorithm, unsigned char *x, uint32 xlen, unsigned char *y, uint32 ylen)
{
	...
	//printf("ecc curid=%d\n",publicKey->key->ecc.dp->curveId);

	switch(publicKey->key->ecc.dp->curveId)
	{
		case 19://192R1
			curve_oid = "\x2A\x86\x48\xCE\x3D\x03\x01\x01";
			curve_oidlen = 8;
		break;
		case 21://224R1
			curve_oid = "\x2B\x81\x04\x00\x21";
			curve_oidlen = 5;
		break;
		case 23://256R1
			curve_oid = "\x2A\x86\x48\xCE\x3D\x03\x01\x07";
			curve_oidlen = 8;
		break;
		case 24://384R1
			curve_oid = "\x2B\x81\x04\x00\x22";
			curve_oidlen = 5;
		break;
		case 25://521R1
			curve_oid = "\x2B\x81\x04\x00\x23";
			curve_oidlen = 5;
		break;
	}
		

	oid = "\x2A\x86\x48\xCE\x3D\x02\x01";
	oidlen = 7;
	__ASN_CHECK__(len,setAsnRawData(p,start,y,ylen));
	//printf("y raw len=%d\n",len);
	__ASN_CHECK__(len,setAsnRawData(p,start,x,xlen));
	//printf("x raw len=%d\n",len);
	__ASN_CHECK__( len, setAsnTag( p, start, 0x04 ) );
	//printf("tag raw len=%d\n",len);
	__ASN_CHECK__( len, setAsnTag( p, start, 0x00 ) );
	//printf("tag raw len=%d\n",len);
	__ASN_CHECK__( len, setAsnLength( p, start, len ) );
	//printf("key len=%d\n",len);
	...
	__ASN_CHECK__( len, x509_write_ecc_oid( p, start, oid, oidlen, curve_oid, curve_oidlen ) );
	//printf("oid raw len=%d\n",len);
	//__ASN_CHECK__( len, setAsnLength( p, start, len ) );
	//printf("len raw len=%d\n",len);
	__ASN_CHECK__( len, setAsnSequence( p, start, len ) );
	//printf("seq raw len=%d\n",len);
	return len;
}


static int x509_write_extend_key_usage( unsigned char **p, unsigned char *start,
		uint32 ekuFlags, unsigned char *oid, uint32 oid_len, int critical ) {

	int ret, i;
	uint32 len = 0;
	struct _ext_key_usage_ {
		int32 oid, flag;
	} eku[] = {
		{ EKU_TLS_SERVER_AUTH, EXT_KEY_USAGE_TLS_SERVER_AUTH },
		{ EKU_TLS_CLIENT_AUTH, EXT_KEY_USAGE_TLS_CLIENT_AUTH },
		{ EKU_CODE_SIGNING, EXT_KEY_USAGE_CODE_SIGNING },
		{ EKU_EMAIL_PROTECTION, EXT_KEY_USAGE_EMAIL_PROTECTION },
		{ EKU_TIME_STAMPING, EXT_KEY_USAGE_TIME_STAMPING },
		{ EKU_OCSP_SIGNING, EXT_KEY_USAGE_OCSP_SIGNING },
	};

	for (i = 0; i < (sizeof(eku)/sizeof(eku[0])); i++)
	{
		if (eku[i].flag && ekuFlags)
		{
			__ASN_CHECK__( len, setAsnOID( p, start, (unsigned char *)&eku[i].oid, sizeof(eku[i].oid) ) );
		}
	}
...
#if 0
static int x509_write_tvm_name( unsigned char **p, unsigned char *start,
		unsigned char *tvm_name, uint32 tvm_name_len, unsigned char *oid, uint32 oid_len, int critical ) {

	int ret;
	uint32 len = 0;

	if ( tvm_name != NULL && tvm_name_len > 0 )
	{
		__ASN_CHECK__( len, setAsnRawData( p, start, tvm_name, tvm_name_len ) );
	}
...
static int x509_write_count( unsigned char **p, unsigned char *start,
		uint32 counter, unsigned char *oid, uint32 oid_len, int critical ) {
...
static int x509_write_extensions( unsigned char **p, unsigned char *start,
    ...
    int ret;
    uint32 len = 0;
	DT_LOG_INF("write_extensions start1");
    __ASN_CHECK__( len, x509_write_basic_constraints( p, start, &ext->bc, (unsigned char *)"\x55\x1D\x13",
    		0x03, (ext->keyUsageFlags & EXT_CRIT_FLAG(EXT_BASIC_CONSTRAINTS)) ) );
	DT_LOG_INF("write_extensions start2");
    __ASN_CHECK__( len, x509_write_subject_name( p, start, ext->san, (unsigned char *)"\x55\x1D\x11",
    		0x03, (ext->keyUsageFlags & EXT_CRIT_FLAG(EXT_ALT_SUBJECT_NAME)) ) );
	DT_LOG_INF("write_extensions start3");
    __ASN_CHECK__( len, x509_write_subject_key_id( p, start, &ext->sk, (unsigned char *)"\x55\x1D\x0E",
    		0x03, (ext->keyUsageFlags & EXT_CRIT_FLAG(EXT_SUBJ_KEY_ID)) ) );
	DT_LOG_INF("write_extensions start4");
    __ASN_CHECK__( len, x509_write_auth_key_id( p, start, &ext->ak, (unsigned char *)"\x55\x1D\x23",
    		0x03, (ext->keyUsageFlags & EXT_CRIT_FLAG(EXT_AUTH_KEY_ID)) ) );
	DT_LOG_INF("write_extensions start5");
    __ASN_CHECK__( len, x509_write_key_usage( p, start, ext->keyUsageFlags, (unsigned char *)"\x55\x1D\x0F",
    		0x03, (ext->keyUsageFlags & EXT_CRIT_FLAG(EXT_KEY_USAGE)) ) );
	DT_LOG_INF("write_extensions start6");
    __ASN_CHECK__( len, x509_write_extend_key_usage( p, start, ext->ekuFlags, (unsigned char *)"\x55\x1D\x25",
    		0x03, (ext->keyUsageFlags & EXT_CRIT_FLAG(EXT_EXTND_KEY_USAGE)) ) );
	DT_LOG_INF("write_extensions start7");
    __ASN_CHECK__( len, setAsnSequence( p, start, len ) );
    __ASN_CHECK__( len, setAsnLength( p, start, len ) );
	__ASN_CHECK__( len, setAsnTag( p, start, (ASN_CONTEXT_SPECIFIC | ASN_CONSTRUCTED | EXPLICIT_EXTENSION) ) );
	DT_LOG_INF("write_extensions start8");
    return( (int) len );
}

static int x509_write_signature( psPool_t *pool, psPubKey_t *privKey,
		unsigned char *certStart, uint32 certLen, int32 sigAlgorithm,
		unsigned char *out, uint32 outlen)
{
	...
	/*
		Certificate signature info
	*/
    if (sigAlgorithm == OID_SHA1_RSA_SIG || sigAlgorithm == OID_SHA256_RSA_SIG ||
		sigAlgorithm == OID_SHA384_RSA_SIG || sigAlgorithm == OID_SHA512_RSA_SIG ||
		sigAlgorithm == OID_MD2_RSA_SIG || sigAlgorithm == OID_MD5_RSA_SIG)
    {
    	if (sigAlgorithm == OID_SHA1_RSA_SIG) {
    		psSha1Init(&hashCtx);
    		psSha1Update(&hashCtx, certStart, certLen);
    		psSha1Final(&hashCtx, sigHash);
    		sigHashLen = SHA1_HASH_SIZE;
    	}
    	if (sigAlgorithm == OID_SHA256_RSA_SIG) {
    		psSha256Init(&hashCtx);
    		psSha256Update(&hashCtx, certStart, certLen);
    		psSha256Final(&hashCtx, sigHash);
    		sigHashLen = SHA256_HASH_SIZE;
    	}
    	if (sigAlgorithm == OID_SHA384_RSA_SIG) {
    		psSha384Init(&hashCtx);
    		psSha384Update(&hashCtx, certStart, certLen);
    		psSha384Final(&hashCtx, sigHash);
    		sigHashLen = SHA384_HASH_SIZE;
    	}
    	if (sigAlgorithm == OID_SHA512_RSA_SIG) {
    		psSha512Init(&hashCtx);
    		psSha512Update(&hashCtx, certStart, certLen);
    		psSha512Final(&hashCtx, sigHash);
    		sigHashLen = SHA512_HASH_SIZE;
    	}
#ifdef USE_MD2
    	if (sigAlgorithm == OID_MD2_RSA_SIG) {
			psMd2Init(&hashCtx);
			psMd2Update(&hashCtx, certStart, certLen);
			psMd2Final(&hashCtx, sigHash);
			sigHashLen = MD5_HASH_SIZE;
    	}
#endif
#ifdef USE_MD5
    	if (sigAlgorithm == OID_MD5_RSA_SIG) {
    		psMd5Init(&hashCtx);
    		psMd5Update(&hashCtx, certStart, certLen);
    		psMd5Final(&hashCtx, sigHash);
    		sigHashLen = MD5_HASH_SIZE;
...
#endif
    	ret = privRsaEncryptSignedElement(pool, privKey,
    				sigHash, sigHashLen, signature, sizeof(signature), 0);
    	if ( ret < 0 ) {
    		return ret;
    	}
    ...
    __ASN_CHECK__( len, setAsnBitString( &c, tmp_buf, signature, signatureLen * 8 ) );
	__ASN_CHECK__( len, x509_write_alorithm_identifier( &c, tmp_buf, sigAlgorithm ) );
...
int32 psX509ExportCertBinary( psPool_t *pool, psX509Cert_t *cert,
		psPubKey_t *privkey, unsigned char *out, uint32 outlen )
{
	int ret = 0;
	uint32 len = 0;
	unsigned char *c, *c2;
	unsigned char tmp_buf[1024 * 4];

	uint32 signlen = 0;
	unsigned char sign[512];

	c = c2 = tmp_buf + sizeof( tmp_buf ) -1024;
    ...
    /*
		...
		extensions		[3]	EXPLICIT Extensions OPTIONAL
							-- If present, version shall be v3	}
    */
    //DT_LOG_INF("extensions=%d",len);
    DT_LOG_INF("~~~~~~start to generate cert~~~~~~~");
    __ASN_CHECK__( len, x509_write_extensions( &c, tmp_buf, &cert->extensions ) );
	DT_LOG_INF("extensions=%d",len);
	__ASN_CHECK__( len, x509_write_publickey( pool, &c, tmp_buf, &cert->publicKey, cert->pubKeyAlgorithm ) );
	DT_LOG_INF("publicKey=%d",len);
	
	__ASN_CHECK__( len, x509_write_attributes( &c, tmp_buf, &cert->subject ) );
	DT_LOG_INF("subject=%d",len);

	__ASN_CHECK__( len, x509_write_time( &c, tmp_buf, cert->notBefore, cert->notBeforeTimeType, cert->notAfter, cert->notAfterTimeType ) );
	DT_LOG_INF("notBefore=%d",len);

	__ASN_CHECK__( len, x509_write_attributes( &c, tmp_buf, &cert->issuer ) );
	DT_LOG_INF("issuer=%d",len);
	__ASN_CHECK__( len, x509_write_alorithm_identifier( &c, tmp_buf, cert->certAlgorithm ) );
	DT_LOG_INF("certAlgorithm=%d",len);

	__ASN_CHECK__( len, x509_write_serial_number( &c, tmp_buf, cert->serialNumber, cert->serialNumberLen ) );
	DT_LOG_INF("serialNumberLen=%d",len);

	__ASN_CHECK__( len, x509_write_version( &c, tmp_buf, cert->version ) );
	DT_LOG_INF("version=%d",len);

	__ASN_CHECK__( len, setAsnSequence(  &c, tmp_buf, len ) );
	DT_LOG_INF("tmp_buf=%d",len);
	__ASN_CHECK__( signlen, x509_write_signature( pool, privkey, c, len, cert->sigAlgorithm, sign, sizeof(sign) ) );
	  DT_LOG_INF("sigAlgorithm=%d",len);
	  memcpy( c2, sign, signlen); len += signlen;
	__ASN_CHECK__( len, setAsnSequence(  &c, tmp_buf, len ) );
	DT_LOG_INF("tmp_buf11111=%d",len);
