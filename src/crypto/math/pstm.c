			...
			pstm_int *Y)
{
    ...
    mbedtls_mpi mX, mA, mE, mN;

    mbedtls_mpi_init( &mX ); mbedtls_mpi_init( &mA );
    mbedtls_mpi_init( &mE ); mbedtls_mpi_init( &mN );

    len = pstm_unsigned_bin_size( G );
    pstm_to_unsigned_bin(pool, G, temp);
    MBEDTLS_MPI_CHK( mbedtls_mpi_read_binary( &mA, temp, len ) );

    len = pstm_unsigned_bin_size( X );
    pstm_to_unsigned_bin(pool, X, temp);
    MBEDTLS_MPI_CHK( mbedtls_mpi_read_binary( &mE, temp, len ) );

    len = pstm_unsigned_bin_size( P );
    pstm_to_unsigned_bin(pool, P, temp);
    MBEDTLS_MPI_CHK( mbedtls_mpi_read_binary( &mN, temp, len ) );

    // X = A^E mod N  (HAC 14.85)
    MBEDTLS_MPI_CHK( mbedtls_mpi_exp_mod( &mX, &mA, &mE, &mN, NULL ) );
    pstm_copy_unsigned_bin(Y, (unsigned char *)mX.p, mbedtls_mpi_size( &mX ));

cleanup:
    mbedtls_mpi_free( &mX ); mbedtls_mpi_free( &mA );
    mbedtls_mpi_free( &mE ); mbedtls_mpi_free( &mN );
...
/******************************************************************************/
/* copy b to a */
int32 pstm_copy_unsigned_bin(pstm_int *a, unsigned char *b, int32 c)
{
	uint32 i = 0;
	unsigned char *dp = 0;
	...
	/* Big enough based on the len? */
	a->used = (c / sizeof(pstm_digit)) + ((c % sizeof(pstm_digit))?(1):(0));
