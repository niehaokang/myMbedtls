...
*/
	if (keyLen > (uint32)padLen) {
		if (hashSize == SHA512_HASH_SIZE) {
#ifdef USE_SHA512
			psSha512Init(&sha);
			psSha512Update(&sha, key, keyLen);
			psSha512Final(&sha, hash);
#else
...
#ifdef USE_SHA224
			psSha224Init(&sha);
			psSha224Update(&sha, key, keyLen);
			psSha224Final(&sha, hash);
#else
			return PS_UNSUPPORTED_FAIL;
#endif
		}
				...
				uint32 hashSize)
{
	...
	if (hashSize == SHA512_HASH_SIZE) {
		...
		psSha512Update(&ctx->u.sha512, ctx->pad, padLen);
#endif
	} else if (hashSize == SHA384_HASH_SIZE) {
#ifdef USE_SHA384
...
#ifdef USE_SHA224
		psSha224Init(&ctx->u.sha256);
		psSha224Update(&ctx->u.sha256, ctx->pad, padLen);
#endif
	}
	for (i = 0; (uint32)i < keyLen; i++) {
		ctx->pad[i] = key[i] ^ 0x5c;
						...
						uint32 len, uint32 hashSize)
{
	...
	if (hashSize == SHA512_HASH_SIZE) {
#ifdef USE_SHA512
		psSha512Update(&ctx->u.sha512, buf, len);
#endif
	} else if (hashSize == SHA384_HASH_SIZE) {
#ifdef USE_SHA384
...
#ifdef USE_SHA224
		psSha224Update(&ctx->u.sha256, buf, len);
#endif
	}
}
						...
						uint32 hashSize)
{
	...
	if (hashSize == SHA512_HASH_SIZE) {
		...
		psSha512Init(&ctx->u.sha512);
		psSha512Update(&ctx->u.sha512, ctx->pad, 128);
		psSha512Update(&ctx->u.sha512, hash, SHA512_HASH_SIZE);
		psSha512Final(&ctx->u.sha512, hash);
#else
		return PS_UNSUPPORTED_FAIL;
#endif
...
#ifdef USE_SHA224
		psSha224Final(&ctx->u.sha256, hash);

		psSha224Init(&ctx->u.sha256);
		psSha224Update(&ctx->u.sha256, ctx->pad, 64);
		psSha224Update(&ctx->u.sha256, hash, SHA224_HASH_SIZE);
		psSha224Final(&ctx->u.sha256, hash);
#else
		return PS_UNSUPPORTED_FAIL;
#endif
