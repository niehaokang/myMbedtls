...
#define mbedtls_aes_context 				psAesKey_t
#define mbedtls_aes_init(C)
#define mbedtls_aes_setkey_enc(C, K, L)		psAesInitKey((K), (L)>>3, (C))
#define mbedtls_aes_crypt_ecb(C, E, S, D)	\
	if ((E))	psAesEncryptBlock((S), (D), C);	\
	else		psAesDecryptBlock((S), (D), C)
#define mbedtls_aes_free(C)
#else
