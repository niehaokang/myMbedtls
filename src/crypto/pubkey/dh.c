...
/*
	...
	keysize: 12, 16, 20, 28, 32, 40, 48, 64 bytes.
*/
int32 psDhKeyGenEx(psPool_t *pool, uint32 keysize, unsigned char *pBin,
					...
					uint32 xbits, psDhKey_t *key, void *data)
{
	...
	*/
	...
	/* init parameters */
	pstm_init( pool, &p ); pstm_init( pool, &g ); pstm_init( pool, &q );
	pstm_init( pool, &minusOne );
	pstm_init( pool, &key->pub ); pstm_init( pool, &key->priv );
	...
	if ((err = pstm_read_unsigned_bin(&q, qBin, qLen)) != PS_SUCCESS) {
		goto error;
	}

	if ( qBin != NULL && qLen > 0 )	pstm_sub_d(pool, &q, 2, &minusOne);
	else							pstm_sub_d(pool, &p, 1, &minusOne);
	...
	for (i = 0; i < 1000; i++) {
		...
		/* constrains the private value to have special bits */
		if ( xbits > 0 ) {
			int l = 0;
			l = xbits % 8;
			buf[0]  |= (1 << (7 - l)); buf[0] <<= l; buf[0] >>= l;
			keysize = (xbits / 8 + ((xbits & 7) != 0 ? 1 : 0));
		}
		...
		/* Test key is between [2 ~ p - 1]/[2 ~ q -2] */
