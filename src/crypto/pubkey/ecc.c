...
*/
	...
	if (psEccX963ImportKey(pool, p, arcLen, pubKey, eccSet) < 0) {
		psTraceCrypto("Unable to parse ECC pubkepsEccMakeKeyExy from cert\n");
		return -1;
	}
	pubKey->dp = eccSet;
	p += arcLen;
...
int32 getEccParamBySize(int32 size, psEccSet_t **set)
{
	int32 i = 0;

	*set = NULL;
	while (eccSets[i].size > 0) {
		if (size == eccSets[i].size) {
			*set = (psEccSet_t*)&eccSets[i];
			return 0;
		}
		i++;
				  ...
				  int32 *bytesWritten, int32 includeSize, void *eccData)
{
	...
	/* If r or s has the high bit set, the ASN.1 encoding should include
		a leading 0x0 byte to prevent it from being "negative".  If the
		output is for TLS usage, the "includeSize" will be set and it is
		NOT necessary to include the 0x0 bytes. */
	if (includeSize == 0) {
		if ((r.dp[r.used - 1] >> (DIGIT_BIT - 1)) & 0x1) {
			rLen++;
			rflag = 1;
		}
		if ((s.dp[s.used - 1] >> (DIGIT_BIT - 1)) & 0x1) {
			sLen++;
			sflag = 1;
		}
	}
...
/**
 ...
 @return PS_SUCCESS if successful
 */
int32 psEccSignHashForkeymaster(psPool_t *pool, unsigned char *in, int32 inlen,
				  unsigned char *c, int32 outlen, psEccKey_t *privKey,
				  int32 *bytesWritten, int32 includeSize, void *eccData)
{
	psEccKey_t		*pubKey;
