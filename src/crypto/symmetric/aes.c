 ...
 */
int32 psCbcAesInit(psCipherContext_t *ctx, unsigned char *IV,
				  unsigned char *key, uint32 keylen)
{
	int32		x, err;

	if (IV == NULL || key == NULL || ctx == NULL) {
		psTraceCrypto("psCbcAesInit arg fail\n");
		return PS_ARG_FAIL;
	}
...
int32 psCbcAesEncrypt(psCipherContext_t *ctx, unsigned char *pt,
					 unsigned char *ct, uint32 len)
{
	...
	if (pt == NULL || ct == NULL || ctx == NULL || (len & 0x7) != 0) {
		psTraceCrypto("Bad parameters to psCbcAesEncrypt\n");
		return PS_ARG_FAIL;
	}
 ...
 */
	if (ctx->aes.blocklen < 0 || (ctx->aes.blocklen >
		...
		psTraceCrypto("Bad blocklen in psCbcAesEncrypt\n");
...
int32 psCbcAesDecrypt(psCipherContext_t *ctx, unsigned char *ct,
					 unsigned char *pt, uint32 len)
{
	...
	if (pt == NULL || ct == NULL || ctx == NULL || (len & 0x7) != 0) {
		psTraceCrypto("Bad parameters to psCbcAesDecrypt\n");
		return PS_ARG_FAIL;
	}
 ...
 */
	if (ctx->aes.blocklen < 0 || (ctx->aes.blocklen >
			(int32)sizeof(ctx->aes.IV))) {
		psTraceCrypto("Bad blocklen in psCbcAesDecrypt\n");
		return PS_LIMIT_FAIL;
	}
	for (i = 0; i < len; i += ctx->aes.blocklen) {
/*
...
#ifdef USE_AES_CTR
/*
	Initialize an AES CTR128 context
*/
int32 psAesInitCTR(psCipherContext_t *ctx, unsigned char *key,
		int32 keylen, unsigned char IV[])
{

	memset(&ctx->aes, 0x0, sizeof(psAesCipher_t));
	if (psAesInitKey(key, keylen, &ctx->aes.key) < 0) {
		return -1;
	}

	memcpy(&ctx->aes.IV[0], IV, 16); /* nonce(4) + iv(8) + counter(4)*/

	return PS_SUCCESS;
}

int32 psAesSetParamsCTR(psCipherContext_t *ctx,
		unsigned int used_bytes_num, unsigned char IV[16])
{
	/* nonce(4) + iv(8) + counter(4)*/
	memcpy(&ctx->aes.IV[0], IV, 16);
	ctx->aes.UsedBlockNumber = used_bytes_num;

	/* when it used_block_num = 0, calculate in psAesEncryptCTR. */
	if (used_bytes_num != 0) {
		psAesEncryptBlock(ctx->aes.IV,
				ctx->aes.EncryptedCount, &ctx->aes.key);
		ctr128_inc(ctx->aes.IV);
	}
	return PS_SUCCESS;
}

/*
	Encrypt data with AES-CTR128
*/
int32 psAesEncryptCTR(psCipherContext_t *ctx, unsigned char *ct,
		unsigned char *pt, uint32 len)
{
	int32			l = 0;
	int32			n = 0;

	n = ctx->aes.UsedBlockNumber;
	...
	while (l < len) {
		...
		pt[l] = ct[l] ^ ctx->aes.EncryptedCount[n];
		++l; n = (n+1) % 16;
	}

	ctx->aes.UsedBlockNumber = n;
	return PS_SUCCESS;
}
#endif /* USE_AES_CTR */
#endif /* USE_AES */
